...
QString ensureActiveShift(Database& database) {
    ...
    auto query=database.prepare("INSERT INTO shift_sessions(id,opened_at,opening_cash_paisa,status) VALUES(?,?,0,'open')"); query.bind(1,id); query.bind(2,utcNow()); query.execute(); tx.commit();
