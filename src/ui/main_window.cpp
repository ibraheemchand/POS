...
QWidget* MainWindow::makeDashboard(){
    ...
    const auto today=QDate::currentDate(); const auto todayStart=today.toString(Qt::ISODate), tomorrow=today.addDays(1).toString(Qt::ISODate);
    pos::BusinessSummary todayReport; try { todayReport=pos::ReportService(database_).summary(today,today); } catch(...) {}
    qint64 receivables=0,lowStock=0,cashToday=0,salesCount=0,expiring=0;
    try { auto r=database_->prepare("SELECT COALESCE(SUM(balance_paisa),0) FROM customers WHERE is_deleted=0"); r.stepRow(); receivables=r.integer(0); auto ls=database_->prepare("SELECT COUNT(*) FROM products WHERE is_deleted=0 AND stock_quantity<=minimum_stock"); ls.stepRow(); lowStock=ls.integer(0); auto ct=database_->prepare("SELECT COALESCE(SUM(total_paisa),0) FROM sales WHERE status!='voided' AND payment_method='cash' AND created_at>=? AND created_at<?"); ct.bind(1,todayStart); ct.bind(2,tomorrow); ct.stepRow(); cashToday=ct.integer(0); auto sc=database_->prepare("SELECT COUNT(*) FROM sales WHERE status!='voided' AND created_at>=? AND created_at<?"); sc.bind(1,todayStart); sc.bind(2,tomorrow); sc.stepRow(); salesCount=sc.integer(0); auto ex=database_->prepare("SELECT COUNT(*) FROM batches WHERE quantity_remaining>0 AND expiry_date IS NOT NULL AND expiry_date>=? AND expiry_date<=?"); ex.bind(1,today.toString(Qt::ISODate)); ex.bind(2,today.addDays(30).toString(Qt::ISODate)); ex.stepRow(); expiring=ex.integer(0); } catch(...) {}
    auto* metrics=new QGridLayout; metrics->setHorizontalSpacing(14); metrics->setVerticalSpacing(14);
    const QList<QList<QString>> rows={{"Today's sales",QString("PKR %1").arg(formatPaisa(todayReport.sales)),salesCount==0?"No sales yet":QString("%1 sale(s) completed").arg(salesCount),"#2563EB"},{"Today's cash",QString("PKR %1").arg(formatPaisa(cashToday)),"Cash taken today","#22C55E"},{"Receivables",QString("PKR %1").arg(formatPaisa(receivables)),"Customer balances outstanding","#F59E0B"},{"Low stock",QString("%1 items").arg(lowStock),lowStock==0?"Inventory healthy":"Needs attention","#EF4444"},{"Today's purchases",QString("PKR %1").arg(formatPaisa(todayReport.purchases)),"Stock received today","#7C3AED"},{"Expiring batches",QString("%1 batches").arg(expiring),expiring==0?"Nothing in the next 30 days":"Due within 30 days","#EA580C"}};
    for(int i=0;i<rows.size();++i)metrics->addWidget(makeMetric(rows[i][0],rows[i][1],rows[i][2],rows[i][3]),i/3,i%3); l->addLayout(metrics);
    ...
    auto* qt=new QLabel("Quick actions",quickPanel); qt->setObjectName("sectionTitle"); auto* qhint=new QLabel("Jump straight to the task you need — no hunting through menus.",quickPanel); qhint->setObjectName("muted");
    ...
    auto* grid=new QGridLayout; grid->setSpacing(10); for(int i=0;i<actions.size();++i){ auto* b=new QPushButton(actions[i],quickPanel); b->setObjectName("quick"); const auto target=targets[i]; connect(b,&QPushButton::clicked,this,[this,target]{goToPage(target);}); grid->addWidget(b,i/4,i%4); } ql->addLayout(grid); l->addWidget(quickPanel);
    ...
    try { auto s=database_->prepare("SELECT invoice_no,total_paisa,created_at FROM sales WHERE status!='voided' ORDER BY created_at DESC LIMIT 4"); while(s.stepRow()) recent->addItem(QString("%1   •   PKR %2   •   %3").arg(s.text(0),formatPaisa(s.integer(1)),s.text(2).left(16))); auto p=database_->prepare("SELECT invoice_no,total_paisa,purchased_at FROM purchases WHERE status='completed' ORDER BY purchased_at DESC LIMIT 4"); while(p.stepRow()) recent->addItem(QString("%1   •   PKR %2   •   %3").arg(p.text(0),formatPaisa(p.integer(1)),p.text(2).left(16))); } catch(...) {}
    al->addWidget(recent,1); lower->addWidget(activity,3);
    auto* next=new QFrame(page); next->setObjectName("panel"); auto* nl=new QVBoxLayout(next); nl->setContentsMargins(20,18,20,18); auto* nt=new QLabel("Attention needed",next); nt->setObjectName("sectionTitle"); auto* notice=new QLabel("✓  Database status: healthy\n\n✓  Offline mode is active\n\n•  Configure automatic backups in Settings",next); notice->setObjectName("muted"); nl->addWidget(nt); nl->addSpacing(14); nl->addWidget(notice); nl->addStretch(); lower->addWidget(next,2);
    l->addLayout(lower,1);
...
QWidget* MainWindow::makeSalesPos(){
    ...
    auto* header=new QFrame(page); header->setObjectName("panel"); auto* hl=new QVBoxLayout(header); hl->setContentsMargins(20,18,20,18); hl->setSpacing(12);
    ...
    auto* customer=new QComboBox(header); customer->setAccessibleName("Customer"); customer->addItem("— Walk-in customer —",QString());
    ...
    auto* columns=new QHBoxLayout; columns->setSpacing(16);
    auto* productPanel=new QFrame(page); productPanel->setObjectName("panel"); auto* productLayout=new QVBoxLayout(productPanel); productLayout->setContentsMargins(18,18,18,18); productLayout->setSpacing(12);
    ...
    auto* products=new QTableWidget(productPanel); products->setColumnCount(5); products->setHorizontalHeaderLabels({"Product","SKU","Stock","Price","Unit"}); products->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch); products->setSelectionBehavior(QAbstractItemView::SelectRows); products->setEditTriggers(QAbstractItemView::NoEditTriggers); products->setAlternatingRowColors(true);
    ...
    auto* cartPanel=new QFrame(page); cartPanel->setObjectName("panel"); auto* cartLayout=new QVBoxLayout(cartPanel); cartLayout->setContentsMargins(18,18,18,18); cartLayout->setSpacing(12);
    ...
    auto* cart=new QTableWidget(cartPanel); cart->setColumnCount(4); cart->setHorizontalHeaderLabels({"Product","Quantity","Unit price","Line total"}); cart->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch); cart->setEditTriggers(QAbstractItemView::NoEditTriggers); cart->setSelectionBehavior(QAbstractItemView::SelectRows); cart->setAlternatingRowColors(true);
    ...
    auto* summary=new QFrame(cartPanel); summary->setObjectName("summaryBox"); auto* sl=new QVBoxLayout(summary); sl->setContentsMargins(16,14,16,14); sl->setSpacing(6);
    ...
    sl->addLayout(subRow); sl->addLayout(discRow); sl->addSpacing(6); sl->addWidget(total);
    ...
    auto* savePrint=new QPushButton("SAVE & PRINT",cartPanel); savePrint->setObjectName("primary"); savePrint->setMinimumHeight(42);
    ...
    load(); connect(search,&QLineEdit::textChanged,this,[load]{load();}); connect(search,&QLineEdit::returnPressed,this,[products,addToCart]{if(products->rowCount()>0){products->selectRow(0);addToCart();}}); connect(products,&QTableWidget::itemDoubleClicked,this,[addToCart]{addToCart();}); connect(add,&QPushButton::clicked,this,addToCart);
